...
const int NN = 26;  // number of neighbors for each cube
...
struct CoordinateHash {
    std::size_t operator()(const Coordinate& coordinate) const {
        return ((std::hash<int>()(coordinate.x) ^ (std::hash<int>()(coordinate.y) << 1)) >> 1) ^ (std::hash<int>()(coordinate.z) << 1);
    }
};
...
class Cube {
    ...
    public:
        Cube(int x, int y, int z, bool state = true) {
            coordinate.x = x;
            coordinate.y = y;
            coordinate.z = z;
            this->state = state;
    ...
    public:
            ...
            for (int i = -1; i < 2; i++) {
                for (int j = -1; j < 2; j++) {
                    for (int k = -1; k < 2; k++) {
                        ...
                        neighbors[c] = Coordinate {coordinate.x + i, coordinate.y + j, coordinate.z + k};
                        c++;
                    }
                }
            }
        ...
        bool operator==(const Cube& other) const {
            return ((coordinate.x == other.coordinate.x) && (coordinate.y == other.coordinate.y) && (coordinate.z == other.coordinate.z));
        }
...
class Grid {
        ...
        Grid(std::ifstream& ifs) {
            ...
            while (std::getline(ifs, line)) {
                ...
                for (auto c: line) {
                    if (c == '#') {
                        coordinate = {x, y, 0}; 
                        cubes[coordinate] = Cube(x, y, 0, true);  // assume z = 0
                    }
                    x++;
                }
                y++;
    ...
    void update(void) {
        ...
        for (auto coordinate: to_activate) {
            cubes[coordinate] = Cube(coordinate.x, coordinate.y, coordinate.z, true);
        }
    }
...
std::ostream& operator<<(std::ostream& os, Grid& grid) {
    for (auto [coordinate, cube] : grid.cubes) {
        os << "active cube @ (" << cube.coordinate.x << ", " << cube.coordinate.y << ", " << cube.coordinate.z << ")" << std::endl;
    }
    return os;
}
